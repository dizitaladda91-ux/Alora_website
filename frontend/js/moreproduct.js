...
function renderProductCatalog(products) {
    ...
    gridContainer.innerHTML = products.map((product, index) => {
        ...
        // 1. Cloudinary URL Optimization (Dynamic Compression)
        let optimizedImg = product.baseImg;
        if (optimizedImg && optimizedImg.includes('/upload/')) {
            optimizedImg = optimizedImg.replace('/upload/', '/upload/f_auto,q_auto,w_400/');
        }

        // 2. Pehli 2-3 images (Above the fold) ko eager rakhein, baki sabko lazy
