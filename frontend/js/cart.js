...
function selectSize(size, price, mrp, btnEl) {
    ...
    if (mrpEl) mrpEl.innerHTML = mrp ? `<span style="font-family:Arial,'Noto Sans',sans-serif">&#8377;</span>${mrp}` : '';
}
    ...
    try {
        ...
        sliderTrack.innerHTML = displayProducts.map(product => {
            ...
            <div class="product-card flex-none w-[240px] sm:w-[280px] bg-white rounded-2xl p-3.5 sm:p-4 border border-amber-900/10 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between group relative overflow-hidden" 
                ...
                <div class="w-full flex justify-center items-center h-[130px] sm:h-[160px] overflow-hidden relative my-1 bg-[#F8F5EE]/60 rounded-xl sm:rounded-2xl p-2 border border-[#ECE4CE]/40">
                    <a href="/product/${encodeURIComponent(slug)}" class="block w-full h-full flex items-center justify-center">
                        <img src="${fullImgUrl}" alt="${product.name}" style="mix-blend-mode: multiply;" class="h-full w-full object-contain mix-blend-multiply transition-transform duration-300 group-hover:scale-105 filter drop-shadow-sm" onerror="this.onerror=null; this.src='/static/placeholder.png'">
                    </a>
                </div>
                <div class="flex-1 flex flex-col justify-between space-y-1 mb-2">
                    ...
                    <div class="flex items-baseline justify-center gap-1.5 pt-0.5">
                        <span class="product-price font-fraunces font-bold text-[#8B4513] text-base"><span style="font-family:Arial,'Noto Sans',sans-serif">&#8377;</span>${initialPrice}</span>
                        <span class="product-mrp text-[11px] line-through text-slate-400 font-mono">${initialComparePrice ? '<span style="font-family:Arial,\'Noto Sans\',sans-serif">&#8377;</span>' + initialComparePrice : ''}</span>
                    </div>
                </div>
                <div class="pt-1">
